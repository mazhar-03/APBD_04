...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnDistinctJobTitles()
    {
        ...
        var jobs = emps.Select(e => e.Job).ToList();
        var distinctJobs = jobs.Distinct().ToList();
        //
        Assert.Contains("PRESIDENT", distinctJobs);
        Assert.Contains("SALESMAN", distinctJobs);
    }
    ...
    // SQL: SELECT E.EName, D.DName, S.Grade FROM Emp E JOIN Dept D ON
    // E.DeptNo = D.DeptNo JOIN Salgrade S ON E.Sal BETWEEN S.Losal AND S.Hisal;
    [Fact]
